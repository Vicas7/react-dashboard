...
const SidebarOption = ({ to, Icon, title }) => {
  ...
  const isNotActiveStyle = 'flex gap-2 text-gray-600 px-2 py-1 mx-2 rounded-sm mb-1';
  return (
    <NavLink end to={to} className={({ isActive }) => (isActive ? isActiveStyle : isNotActiveStyle)}>
      <Icon />
...
const Sidebar = () => {
  return (
    <div className='fixed top-12 flex flex-col justify-between w-48 h-[calc(100vh-3em)] py-2 bg-gray-50 text-gray-700 border-r '>
      ...
      <div>
        <SidebarOption to='settings' Icon={IoSettingsSharp} title='Settings' />
      </div>
    </div>
  );
};
